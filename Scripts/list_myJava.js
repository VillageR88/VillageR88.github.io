async function displayFileContent() {
    const response = await fetch("Scripts/myJava.txt");
    const text = await response.text();
    const lines = text.split('\n');
    const fileContentElement = document.getElementById('fileContent6');
    ...
    lines.forEach((line) => {
        ...
        a.textContent = `1.7.` + line.slice(6).match(/(?!0)\d*/g).join("") + `. ${line.slice(5).match(/(?!\d).+/g)}`.slice(0, -5);
        ...
        a.addEventListener('click', (event) => {
            ...
            codeTitle.textContent = "My code: " + line.slice(5,-5).match(/(?!\d).+/g);
            ...
            const url = `https://raw.githubusercontent.com/VillageR88/Codewars/main/Java/${originalName}`;
